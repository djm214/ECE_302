...
Bitset::Bitset(const std::string & value) // Constructor that initializes the value with string in the form "00010000"
{
  ...
  for(intmax_t i = 0; i < N; i++)
  {
    ...
    data[i] = value[i]; //copy string to array
  }
}
...
//Sets the value at index to 1
  ...
  void Bitset::toggle(intmax_t index)
  {
    ...
    if(data[index] == '1') //if it was 1, change to 0
    {
      data[index] = '0'; //reset to 0
    } 
    else if(data[index] == '0') //if it was 0, change to 1
    {
      data[index] = '1'; //set to 1
    }
  }
