    ...
    NodeType* treePtr, FunctionType visit)
{
    if (treePtr != 0) 
    {
        ...
        //begin by checking left branches
        preorder(treePtr->leftChildPtr,visit);

        //recursive call will resume here to check for right

        //begin checking the right branches
        preorder(treePtr->rightChildPtr,visit);   
    }
    return; //recursively call for right branches now
};
